...
| **research-topic** — KB · 엄밀성 정의 (J1 · J3) | Prof. Sho Takatori | — | **`요청 예정`** — 2026-09-03 |
| **전체 관리** | Kyu Hwan Choi | — | `확정` |
| **Funding · advising** | Prof. Sho Takatori | — | **`요청 예정`** |

### 확정 상태 열이 있는 이유

이 레포의 기본 규칙 — **판정할 근거가 없으면 통과가 아니다**(철학 ①) — 이 역할 배정에도 걸린다. 남이 맡을 일을 확정으로 적으면 그건 근거 없는 귀속이고, [personas/roster.md](design/personas/roster.md)가 문헌에 대해 금지한 것과 같은 종류의 오류다.
...
**Prof. Takatori가 J3를 맡으면서 동시에 가치 페르소나의 증류 대상이면, `T-014`의 유일한 자동 검사가 무력화된다.**

`trajectory_check`(그 사람의 가치 주장이 실제 논문 궤적과 맞는가)는 **가치 조건이 문헌에서 독립적으로 역산됐을 때만** 정보를 가진다. 본인이 조건을 직접 써 주면 검사는 통과가 보장되고 정보량이 0이 된다.
...
| 무엇 | 누가 | 왜 |
|---|---|---|
| **J3 — 엄밀성 정의** (축 · 통과 조건의 형식 · 원전의 성립 조건) | 본인이 직접 저술 | 엄밀성은 권위가 저술하는 것이 **엄격하게 더 낫다.** 겹침 문제 없음 |
| **가치 페르소나 정의** | **문헌에서 독립 증류** | 본인 답변은 **held-out 대조**로만 쓴다 |

**결과: 목요일 전에 대조군 초안을 써야 하는 이유가 두 개가 됐다.** ① 상의가 검정이 되려면(§6) ② `trivially_satisfied`를 피하려면. → `T-016`
...
| **7** | **실명 페르소나의 윤리·공개 범위** | §8 · `T-035` | 본인이 페르소나 대상이므로 **동의 여부가 실제 제약이다** |
...
**B의 걸림돌이 기술적인 게 아니라 공개 범위라는 점이 핵심이다.** `agentic-microscope`는 2026-08-28에 벤더 자료를 걷어내고 public이 됐고, 이 레포는 실명 페르소나 때문에 private여야 한다(§8). **둘을 한 레포에 두면 더 엄격한 쪽이 이긴다** — 즉 microscope가 다시 private가 된다.
...
- **페르소나 대상 본인의 동의**는 별개 문제다. §6 안건 7.
